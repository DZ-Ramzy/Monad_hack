...
/**
 * react-bits LightTunnel, ported to TypeScript for this project.
 *
 * Two deliberate changes from the upstream source, both so the thing can sit
 * behind the whole site as a `pointer-events: none` layer:
 *   - the pointer is tracked on `window`, not on the canvas, which would never
 *     see an event through a layer that does not take the pointer;
 *   - WebGL2 is checked before anything is built, because the shader is
 *     `#version 300 es` and ogl falls back to WebGL1 in silence.
 */
...
type TunnelContext = { renderer: Renderer; program: Program; mesh: Mesh }
...
const LightTunnel = ({
  ...
  useEffect(() => {
    ...
    // The shader is `#version 300 es`. Without WebGL2 ogl quietly hands back a
    // WebGL1 context and every frame fails to compile, so leave instead.
    ...
    // Bound to `window`, not to the canvas: behind the page this layer is
    // `pointer-events: none`, so the canvas itself is never hovered.
